...
b8 try_to_parse_and_set_rule(
  std::array<langton::rule, 256> &out,
  json_t const &rule,
  u64 const index,
  std::function<void(std::string &&)> const &add_err)
{
  ...
  langton::turn_direction::value_type turn_dir;
  ...
  try {
    ...
    if (out[shade].turn_dir != langton::turn_direction::NIL) {
      // rule already set
  ...
  try {
    ...
    turn_dir = langton::turn_direction::from_char(turn.front());
...
langton::rules_t langton::reverse_rules(langton::rules_t const &rules)
{
  langton::rules_t rules_reversed = {};

  for (u64 i = 0; i < rules.size(); ++i) {
    auto const &r = rules[i];
    if (r.turn_dir != langton::turn_direction::NIL) {
      rules_reversed[r.replacement_shade] = { static_cast<u8>(i), r.turn_dir };
    }
  }

  return rules_reversed;
}
  ...
  std::function<void(std::string &&)> const &add_err)
{
  ...
  state.rules_reversed = langton::reverse_rules(parsed_rules);
  ...
  errors_t const &errors)
{
  ...
  if (std::regex_match(grid_state, std::regex("^fill=[0-9]{1,}$", std::regex_constants::icase))) {
    ...
    if (state.rules[fill_val].turn_dir == langton::turn_direction::NIL) {
...
langton::state langton::parse_state(
  ...
  errors_t &errors)
{
  ...
  [[maybe_unused]] b8 const last_step_res_parse_success = try_to_parse_and_set_enum<
    langton::step_result::value_type
  >(
    json,
    "last_step_result",
    {
      { langton::step_result::to_cstr(langton::step_result::NIL), langton::step_result::NIL },
      { langton::step_result::to_cstr(langton::step_result::SUCCESS), langton::step_result::SUCCESS },
      { langton::step_result::to_cstr(langton::step_result::HIT_EDGE), langton::step_result::HIT_EDGE },
    },
    state.last_step_res,
    add_err
  );
  ...
  [[maybe_unused]] b8 const ant_orientation_parse_success = try_to_parse_and_set_enum<
    langton::orientation::value_type
  >(
    json,
    "ant_orientation",
    {
      { langton::orientation::to_cstr(langton::orientation::NORTH), langton::orientation::NORTH },
      { langton::orientation::to_cstr(langton::orientation::EAST), langton::orientation::EAST },
      { langton::orientation::to_cstr(langton::orientation::SOUTH), langton::orientation::SOUTH },
      { langton::orientation::to_cstr(langton::orientation::WEST), langton::orientation::WEST },
    },
