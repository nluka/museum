...
langton::save_state_result langton::save_state(
  langton::state const &state,
...
bool langton::print_state_json(
  std::ostream &os,
  [[maybe_unused]] std::string const &file_path,
  std::string const &grid_state,
  bool grid_state_is_path,
  ...
  rules_t const &rules)
{
  auto grid_state_str = grid_state_is_path ? fs::path(grid_state).generic_string() : grid_state;

  os
    << "{\n"
    << "  \"generation\": " << generation << ",\n"
    << "  \"last_step_result\": \"" << langton::step_result::to_cstr(last_step_res) << "\",\n"
    << "\n"
    << "  \"grid_width\": " << grid_width << ",\n"
    << "  \"grid_height\": " << grid_height << ",\n"
    << "  \"grid_state\": \"" << grid_state_str << "\",\n"
    << "\n"
    << "  \"ant_col\": " << ant_col << ",\n"
    << "  \"ant_row\": " << ant_row << ",\n"
    << "  \"ant_orientation\": \"" << langton::orientation::to_cstr(ant_orientation) << "\",\n"
    << "\n"
    << "  \"rules\": [\n"
  ;
